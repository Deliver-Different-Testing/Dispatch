...
> **Implementation spec for Dispatch.** §4 is the change to build — the price-breakdown substitution shown to a logged-in network partner. §3 is how network partner pay works in production and why §4 is safe; note §3.3, which splits the pay calculation into two paths with different timing. §5 onward is supporting analysis, and §7 records where earlier revisions of this doc were wrong.
>
> **Before building §4, run Q6/Q7 and Q14** (§8). They decide whether this is a view-only change or a view change plus a data fix, and whether `CourierFuel` is populated enough to bind to.
>
