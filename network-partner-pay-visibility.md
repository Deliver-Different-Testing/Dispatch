...
> **Implementation spec for Dispatch.** §3 is the change to build — the price-breakdown substitution shown to a logged-in network partner. §2 is how network partner pay works in production and why §3 is safe; note §2.3, which splits the pay calculation into two paths with different timing. §4 onward is supporting analysis, and §6 records where earlier revisions of this doc were wrong.
>
> **This is two pieces of work.** §2.5 confirms neither `CourierPayment` nor `CourierFuel` is populated on a network partner job today, so the assignment-time data work must land before or with the display change — the substitution alone would show the partner zero. Confirm the field names (Q15, Q16) before writing code.
>
